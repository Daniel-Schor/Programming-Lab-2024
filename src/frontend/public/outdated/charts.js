...
function pizzaRanking() {
 ...
 /* return new Promise((resolve, reject) => {
    var chartDom = document.getElementById('pizzaRanking');
    var myChart = echarts.init(chartDom);

    fetch(`/api/pizza-ranking`)
      .then((response) => response.json())
      .then((data) => {
        var option = {
          tooltip: {
        ...
        myChart.off('click');
        myChart.on('click', (params) => {
          window.location.href = `/individualPizza?pizzaID=${params.name}`;
          localStorage.setItem('pizza', JSON.stringify({ "pizzaID": params.name })); // Store the pizza variable
        });
